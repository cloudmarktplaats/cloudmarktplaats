<div class="mx-auto max-w-xl space-y-6 rounded border bg-white p-6 shadow">
    <header>
        <h1 class="text-xl font-bold">{{ __('Tweefactor-authenticatie') }}</h1>
        <p class="text-sm text-cmp-muted">
            {{ __('Beveilig je account met een eenmalige TOTP-code uit je authenticator-app (zoals Aegis, 1Password, Bitwarden, Google Authenticator).') }}
        </p>
    </header>
    ...
    @if ($enabled && ! $confirmed && ! $secret)
        ...
        <section class="space-y-3 rounded border border-green-200 bg-green-50 p-3">
            <p class="text-sm text-green-800">{{ __('2FA is actief op je account.') }}</p>
        </section>

        <section class="space-y-3">
            <h2 class="text-lg font-semibold">{{ __('Recovery-codes opnieuw genereren') }}</h2>
            <p class="text-xs text-cmp-muted">
                {{ __('Geef je huidige TOTP-code op om 8 nieuwe codes te genereren. De oude codes worden ongeldig.') }}
            </p>
            <form wire:submit="regenerate" class="space-y-2">
                ...
                <button class="rounded-sm bg-cmp-ink px-3 py-1 text-sm text-white hover:bg-cmp-signal">{{ __('Genereer nieuwe codes') }}</button>
            </form>
        </section>

        <section class="space-y-3">
            <h2 class="text-lg font-semibold text-red-700">{{ __('2FA uitschakelen') }}</h2>
            <form wire:submit="disable" class="space-y-2">
                ...
                <button class="rounded border border-red-300 px-3 py-1 text-sm text-red-700 hover:bg-red-50">{{ __('Uitschakelen') }}</button>
            </form>
        </section>
    ...
    @if ($secret && ! $confirmed)
        <section class="space-y-3">
            <h2 class="text-lg font-semibold">{{ __('Stap 1 — scan de QR-code') }}</h2>
            <div>{!! $this->qrSvg() !!}</div>
            <details class="text-xs text-cmp-muted">
                <summary class="cursor-pointer">{{ __('Of voer het geheim handmatig in') }}</summary>
                <code class="mt-1 block break-all rounded bg-cmp-bg2 p-2 font-mono">{{ $secret }}</code>
            </details>

            <h2 class="text-lg font-semibold">{{ __('Stap 2 — bevestig met een code') }}</h2>
            <form wire:submit="confirm" class="space-y-2">
                <input type="text" inputmode="numeric" autocomplete="one-time-code"
                       wire:model="code" placeholder="123456" maxlength="6"
                       class="w-full rounded-sm border-cmp-border p-2 focus:border-cmp-signal focus:ring-cmp-signal font-mono">
                @error('code') <p class="text-sm text-red-600">{{ $message }}</p> @enderror
                <button class="rounded-sm bg-cmp-ink px-3 py-1 text-sm text-white hover:bg-cmp-signal">{{ __('Bevestig') }}</button>
            </form>
        </section>
    @endif
    ...
    @if ($confirmed && count($recovery))
        <section class="space-y-3 rounded border border-amber-200 bg-amber-50 p-3">
            <h2 class="text-lg font-semibold text-amber-900">{{ __('Bewaar deze recovery-codes') }}</h2>
            <p class="text-xs text-amber-900">
                {{ __('Deze codes zie je maar één keer. Bewaar ze veilig (password manager). Elke code is eenmalig bruikbaar als je geen toegang hebt tot je authenticator-app.') }}
            </p>
    ...
    <p class="text-xs">
        <a href="/profile/security" class="underline">{{ __('Terug naar beveiliging') }}</a>
    </p>
</div>
