<div class="mx-auto max-w-md rounded-sm border border-cmp-border bg-cmp-surface p-6">
    ...
    <form wire:submit="submit" class="space-y-3">
        ...
        <button class="w-full cmp-btn cmp-btn-primary">Inloggen</button>
    ...
    @if (config('cloudmarktplaats.features.oauth_github') || config('cloudmarktplaats.features.oauth_gitlab'))
        ...
        <div class="space-y-2">
            @if (config('cloudmarktplaats.features.oauth_github'))
                <a href="/oauth/github/redirect" class="block w-full rounded-sm border border-cmp-ink px-4 py-2 text-center font-medium hover:bg-cmp-ink hover:text-white transition-colors">Inloggen met GitHub</a>
            @endif
            @if (config('cloudmarktplaats.features.oauth_gitlab'))
                <a href="/oauth/gitlab/redirect" class="block w-full rounded-sm border border-cmp-ink px-4 py-2 text-center font-medium hover:bg-cmp-ink hover:text-white transition-colors">Inloggen met GitLab</a>
            @endif
        </div>
    @endif
</div>
