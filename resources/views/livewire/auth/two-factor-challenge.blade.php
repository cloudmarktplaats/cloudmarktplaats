<div class="mx-auto max-w-md rounded-sm border border-cmp-border bg-cmp-surface p-6">
    <h1 class="mb-4 text-xl font-bold">{{ __('Tweefactor-verificatie') }}</h1>
    <p class="mb-3 text-sm text-cmp-muted">
        {{ __('Vul de 6-cijferige code uit je authenticator-app in. Geen toegang tot je app? Gebruik een van je recovery-codes.') }}
    </p>
    <form wire:submit="submit" class="space-y-3">
        <input
            ...
            wire:model="code"
            placeholder="{{ __('123456 of recovery-code') }}"
        ...
        <button class="w-full cmp-btn cmp-btn-primary">{{ __('Verifiëren') }}</button>
    </form>
</div>
