<div class="rounded-sm border border-cmp-border bg-cmp-surface p-5 sm:p-6">
    @if ($sent)
        <div class="flex items-start gap-3" role="status">
            ...
            <div>
                <p class="font-medium text-cmp-text">{{ __('Bericht verstuurd.') }}</p>
                <p class="mt-1 text-sm text-cmp-muted">
                    {{ __('De verkoper kan je rechtstreeks per e-mail antwoorden. We bewaren de inhoud van je bericht niet.') }}
                </p>
            </div>
        </div>
    @else
        <h2 class="font-display text-lg font-bold tracking-display-tight">{{ __('Stuur de verkoper een bericht') }}</h2>
        <p class="mt-1 text-sm text-cmp-muted">
            {{ __('Geen account nodig. De verkoper ziet jouw e-mailadres pas als je verstuurt, en kan daar rechtstreeks op antwoorden.') }}
        </p>
        ...
        <form wire:submit="send" class="mt-5 space-y-4">
            ...
            <div>
                <label for="cs-email" class="block text-sm font-medium text-cmp-text">{{ __('Jouw e-mailadres') }}</label>
                <input
                    type="email"
                    id="cs-email"
                    wire:model="email"
            ...
            <div>
                <label for="cs-body" class="block text-sm font-medium text-cmp-text">{{ __('Bericht') }}</label>
                <textarea
                    id="cs-body"
                    wire:model="body"
                    rows="4"
                    ...
                    maxlength="2000"
                    class="mt-1 block w-full rounded-md border-cmp-border bg-cmp-bg2 text-cmp-text placeholder:text-cmp-faint focus:border-cmp-blue focus:ring-cmp-blue"
                    placeholder="{{ __('Is dit nog beschikbaar? Kan ik het komen bekijken?') }}"
                ></textarea>
