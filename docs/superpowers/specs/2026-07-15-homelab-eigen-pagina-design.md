...
**Maximaal vier foto's, en de verwerking blijft synchroon.** De job draait nu bewust binnen de transactie — de post wordt pas zichtbaar als de foto er echt is, met dit commentaar in `Feed::submit`: *"de rij wordt pas zichtbaar (published mét echt foto-pad) zodra de pipeline slaagde."* Die atomaire garantie is waardevol: er bestaat nooit een homelab zonder foto. Bij tien foto's houdt synchroon geen stand — tien decodes in één webrequest is tien tot twintig seconden en een gateway-timeout in het verschiet, en dáárom doen advertenties het via de queue. Bij vier blijft de request onder een paar seconden en blijft de garantie overeind. Een rack heeft geen tien foto's nodig; voor, achter en een detail vertellen het verhaal.

De per-foto grens (8 MB) komt uit `config('cloudmarktplaats.photos.max_bytes')` — hetzelfde getal als advertenties, want er zijn al drie plekken waar die grens geldt (nginx, PHP's ini, de validatieregel) en de strengste wint stil. Het **aantal** wijkt af: homelabs krijgen een eigen `config('cloudmarktplaats.photos.homelab_max_count')` van 4, naast de bestaande `max_count` van 10 voor advertenties. Twee getallen, want de reden verschilt (timeout-budget, niet opslag).

**De stille upload-bug is inmiddels op productie gefixt** (commits `5055208`, `5b3e816`, uitgerold 16-07) — maar alleen voor de advertentiewizard. Het homelab-formulier heeft dezelfde behandeling nog niet: een mislukte upload laat het veld stil leeg. Van één naar vier foto's gaan vergroot dat precies daar, dus het homelab-formulier krijgt in deze feature dezelfde behandeling als de wizard kreeg: voortgang, leesbare uploadfouten, en te grote of te veel bestanden meteen gemeld bij het kiezen — gevoed door dezelfde config-waarden.
